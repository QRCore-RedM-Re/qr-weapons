...
AddEventHandler('QRCore:Client:OnPlayerLoaded', function()
    ...
    exports['qr-core']:TriggerCallback("weapons:server:GetConfig", function(RepairPoints)
    ...
    if CurrentWeaponData then
        if sharedWeapons[weapon]["name"] ~= "weapon_unarmed" and sharedWeapons[weapon]["name"] ~= "weapon_bow" and sharedWeapons[weapon]["name"] ~= "weapon_bow_improved" and sharedWeapons[weapon]["ammotype"] == type:upper() then
            ...
            if total + (amount/2) < 200 then
                exports['qr-core']:Progressbar("taking_bullets", Lang:t('info.loading_bullets'), math.random(4000, 6000), false, true, {
                    ...
                    if sharedWeapons[weapon] then
                        ...
                        TriggerServerEvent('QRCore:Server:RemoveItem', itemData.name, 1, itemData.slot)
                        TriggerEvent('inventory:client:ItemBox', sharedItems[itemData.name], "remove")
                        TriggerEvent('QRCore:Notify', Lang:t('success.reloaded'), 5000, 0, 'hud_textures', 'check', 'COLOR_WHITE')
                    end
                end, function()
                    exports['qr-core']:Notify(9, Lang:t('error.canceled'), 5000, 0, 'mp_lobby_textures', 'cross', 'COLOR_WHITE')
                end)
            else
                exports['qr-core']:Notify(9, Lang:t('error.max_ammo'), 5000, 0, 'mp_lobby_textures', 'cross', 'COLOR_WHITE')
            end
        elseif sharedWeapons[weapon]["name"] == "weapon_bow" or sharedWeapons[weapon]["name"] == "weapon_bow_improved" and sharedWeapons[weapon]["ammotype"] == type:upper() then
            ...
            exports['qr-core']:Notify(9, Lang:t('error.no_weapon'), 5000, 0, 'mp_lobby_textures', 'cross', 'COLOR_WHITE')
        end
    else
        exports['qr-core']:Notify(9, Lang:t('error.no_weapon'), 5000, 0, 'mp_lobby_textures', 'cross', 'COLOR_WHITE')
    end
    ...
    if weapon ~= `WEAPON_UNARMED` then
        ...
        if WeaponAttachments[WeaponData.name] then
            if WeaponAttachments[WeaponData.name][attachment]['item'] == ItemData.name then
                ...
                exports['qr-core']:Notify(9, Lang:t('error.no_support_attachment'), 5000, 0, 'mp_lobby_textures', 'cross', 'COLOR_WHITE')
            end
        end
    else
        exports['qr-core']:Notify(9, Lang:t('error.no_weapon_in_hand'), 5000, 0, 'mp_lobby_textures', 'cross', 'COLOR_WHITE')
    end
end)
                    ...
                    if CanShoot then
                        if weapon and weapon ~= 0 and sharedWeapons[weapon] then
                            ...
                            if sharedWeapons[weapon]["name"] == "weapon_snowball" then
                                TriggerServerEvent('QRCore:Server:RemoveItem', "snowball", 1)
                            -- elseif sharedWeapons[weapon]["name"] == "weapon_pipebomb" then
                                -- TriggerServerEvent('QRCore:Server:RemoveItem', "weapon_pipebomb", 1)
                            -- elseif sharedWeapons[weapon]["name"] == "weapon_molotov" then
                                -- TriggerServerEvent('QRCore:Server:RemoveItem', "weapon_molotov", 1)
                            -- elseif sharedWeapons[weapon]["name"] == "weapon_stickybomb" then
                                -- TriggerServerEvent('QRCore:Server:RemoveItem', "weapon_stickybomb", 1)
                            -- elseif sharedWeapons[weapon]["name"] == "weapon_grenade" then
                                -- TriggerServerEvent('QRCore:Server:RemoveItem', "weapon_grenade", 1)
                            -- elseif sharedWeapons[weapon]["name"] == "weapon_bzgas" then
                                -- TriggerServerEvent('QRCore:Server:RemoveItem', "weapon_bzgas", 1)
                            -- elseif sharedWeapons[weapon]["name"] == "weapon_proxmine" then
                                -- TriggerServerEvent('QRCore:Server:RemoveItem', "weapon_proxmine", 1)
                            -- elseif sharedWeapons[weapon]["name"] == "weapon_ball" then
                                -- TriggerServerEvent('QRCore:Server:RemoveItem', "weapon_ball", 1)
                            -- elseif sharedWeapons[weapon]["name"] == "weapon_smokegrenade" then
                                -- TriggerServerEvent('QRCore:Server:RemoveItem', "weapon_smokegrenade", 1)
                            -- elseif sharedWeapons[weapon]["name"] == "weapon_flare" then
                                -- TriggerServerEvent('QRCore:Server:RemoveItem', "weapon_flare", 1)
                            else
                                if ammo > 0 then
                        ...
                        if weapon ~= -1569615261 then
                            ...
                            exports['qr-core']:Notify(9, Lang:t('error.weapon_broken'), 5000, 0, 'mp_lobby_textures', 'cross', 'COLOR_WHITE')
                            MultiplierAmount = 0
                        end
                    end
                end
                            ...
                            if CurrentWeaponData and next(CurrentWeaponData) then
                                if not data.RepairingData.Ready then
                                    ...
                                    if IsControlJustReleased(0, 0xCEFD9220) then
                                        exports['qr-core']:TriggerCallback('weapons:server:RepairWeapon', function(HasMoney)
                                            if HasMoney then
                                                CurrentWeaponData = {}
                                            end
